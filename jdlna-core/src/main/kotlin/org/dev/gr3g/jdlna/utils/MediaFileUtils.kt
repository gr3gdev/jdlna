...
object MediaFileUtils {
    ...
    fun update() {
        val folderVideos = DatabaseDAO.selectConf(DatabaseDAO.FOLDER_VIDEO)
        val folderMusics = DatabaseDAO.selectConf(DatabaseDAO.FOLDER_MUSIC)
        val folderPhotos = DatabaseDAO.selectConf(DatabaseDAO.FOLDER_IMAGE)
        init(Paths.get(folderVideos), Paths.get(folderPhotos), Paths.get(folderMusics))
    }
    ...
    @Throws(IOException::class)
    fun init(videoPath: Path?, imagePath: Path?, musicPath: Path?) {
